...
mod tests {
    ...
    mod t_add {
        ...
        fn can_use_shortcut_both_zero() {
            can_shorcut::<add::Zero, HalfSize>(0, 0, Some(()), Some(()));
        }
        #[test]
        fn can_use_shortcut_rhs_zero() {
            can_shorcut::<add::Zero, HalfSize>(NON_ZERO, 0, None, Some(()));
        }
        #[test]
        fn can_use_shortcut_lhs_zero() {
            can_shorcut::<add::Zero, HalfSize>(0, NON_ZERO, Some(()), None);
        }
        #[test]
        fn can_use_shortcut_none_zero() {
            can_shorcut::<add::Zero, HalfSize>(NON_ZERO, NON_ZERO, None, None);
        }

        #[test]
        fn use_shortcut_lhs_zero() {
            test_shorcut::<add::Zero, Left, HalfSize>(0, NON_ZERO, (), NON_ZERO, OP_DBG);
        }
        #[test]
        fn use_shortcut_rhs_zero() {
            test_shorcut::<add::Zero, Right, HalfSize>(NON_ZERO, 0, (), NON_ZERO, OP_DBG);
        }
        #[test]
        fn use_shortcut_both_zero() {
            test_shorcut::<add::Zero, Left, HalfSize>(0, 0, (), 0, OP_DBG);
            test_shorcut::<add::Zero, Right, HalfSize>(0, 0, (), 0, OP_DBG);
        }
    }
    mod t_sub {
        const OP_DBG: &str = "-";
        ...
        #[test]
        fn can_use_shortcut_both_zero() {
            can_shorcut::<sub::Zero, HalfSize>(0, 0, Some(()), Some(()));
        }
        #[test]
        fn can_use_shortcut_rhs_zero() {
            can_shorcut::<sub::Zero, HalfSize>(NON_ZERO, 0, None, Some(()));
        }
        #[test]
        fn can_use_shortcut_lhs_zero() {
            can_shorcut::<sub::Zero, HalfSize>(0, NON_ZERO, Some(()), None);
        }
        #[test]
        fn can_use_shortcut_none_zero() {
            can_shorcut::<sub::Zero, HalfSize>(NON_ZERO, NON_ZERO, None, None);
        }

        #[test]
        fn use_shortcut_lhs_zero() {
            test_shorcut::<sub::Zero, Left, HalfSize>(NON_ZERO, 0, (), -NON_ZERO, OP_DBG);
        }
        #[test]
        fn use_shortcut_rhs_zero() {
            test_shorcut::<sub::Zero, Right, HalfSize>(NON_ZERO, 0, (), NON_ZERO, OP_DBG);
        }
        #[test]
        fn use_shortcut_both_zero() {
            test_shorcut::<sub::Zero, Left, HalfSize>(0, 0, (), 0, OP_DBG);
            test_shorcut::<sub::Zero, Right, HalfSize>(0, 0, (), 0, OP_DBG);
        }
    }
    ...
    mod t_mul {
        ...
        mod zero {
            ...
            #[test]
            fn can_use_shortcut_both_zero() {
                can_shorcut::<mul::ByZero, HalfSize>(0, 0, Some(()), Some(()));
            }
            #[test]
            fn can_use_shortcut_rhs_zero() {
                can_shorcut::<mul::ByZero, HalfSize>(NON_ZERO, 0, None, Some(()));
            }
            #[test]
            fn can_use_shortcut_lhs_zero() {
                can_shorcut::<mul::ByZero, HalfSize>(0, NON_ZERO, Some(()), None);
            }
            #[test]
            fn can_use_shortcut_none_zero() {
                can_shorcut::<mul::ByZero, HalfSize>(NON_ZERO, NON_ZERO, None, None);
            }

            #[test]
            fn use_shortcut_lhs_zero() {
                test_shorcut::<mul::ByZero, Left, HalfSize>(0, NON_ZERO, (), 0, super::OP_DBG);
            }
            #[test]
            fn use_shortcut_rhs_zero() {
                test_shorcut::<mul::ByZero, Right, HalfSize>(NON_ZERO, 0, (), 0, super::OP_DBG);
            }
            #[test]
            fn use_shortcut_both_zero() {
                test_shorcut::<mul::ByZero, Left, HalfSize>(0, 0, (), 0, super::OP_DBG);
                test_shorcut::<mul::ByZero, Right, HalfSize>(0, 0, (), 0, super::OP_DBG);
            }
        }
        mod one {
            use super::super::*;
            ...
            fn can_use_shortcut_both_one() {
                can_shorcut::<mul::ByOne, HalfSize>(1, 1, Some(()), Some(()));
            }
            #[test]
            fn can_use_shortcut_rhs_one() {
                can_shorcut::<mul::ByOne, HalfSize>(NON_ONE, 1, None, Some(()));
            }
            #[test]
            fn can_use_shortcut_lhs_one() {
                can_shorcut::<mul::ByOne, HalfSize>(1, NON_ONE, Some(()), None);
            }
            #[test]
            fn can_use_shortcut_none_one() {
                can_shorcut::<mul::ByOne, HalfSize>(NON_ONE, NON_ONE, None, None);
            }

            #[test]
            fn use_shortcut_lhs_one() {
                test_shorcut::<mul::ByOne, Left, HalfSize>(1, NON_ONE, (), NON_ONE, super::OP_DBG);
            }
            #[test]
            fn use_shortcut_rhs_one() {
                test_shorcut::<mul::ByOne, Right, HalfSize>(NON_ONE, 1, (), NON_ONE, super::OP_DBG);
            }
            #[test]
            fn use_shortcut_both_one() {
                test_shorcut::<mul::ByOne, Left, HalfSize>(1, 1, (), 1, super::OP_DBG);
                test_shorcut::<mul::ByOne, Right, HalfSize>(1, 1, (), 1, super::OP_DBG);
            }
        }
        ...
        mod pow2 {
            ...
            fn can_use_shortcut_both() {
                can_shorcut::<mul::ByPowerOfTwo, HalfSize>(POW2, POW2, Some(()), Some(()));
            }
            #[test]
            fn can_use_shortcut_rhs() {
                can_shorcut::<mul::ByPowerOfTwo, HalfSize>(NON_POW2, POW2, None, Some(()));
            }
            #[test]
            fn can_use_shortcut_lhs() {
                can_shorcut::<mul::ByPowerOfTwo, HalfSize>(POW2, NON_POW2, Some(()), None);
            }
            #[test]
            fn can_use_shortcut_none() {
                can_shorcut::<mul::ByPowerOfTwo, HalfSize>(NON_POW2, NON_POW2, None, None);
            }

            #[test]
            fn use_shortcut_lhs() {
                test_shorcut::<mul::ByPowerOfTwo, Left, HalfSize>(
                    POW2,
                    NON_POW2,
                    (),
            ...
            #[test]
            fn use_shortcut_rhs() {
                test_shorcut::<mul::ByPowerOfTwo, Right, HalfSize>(
                    NON_POW2,
                    POW2,
                    (),
                    (NON_POW2 as u32) << (POW as u32),
            ...
            #[test]
            fn mul_sign_pow_two() {
                test_shorcut_commte::<mul::ByPowerOfTwo, Left, HalfSize>(
                    2,
                    2,
                    (),
                    4,
